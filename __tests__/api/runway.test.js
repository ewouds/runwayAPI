...
describe('Runway API', () => {
  ...
  describe('Successful airport data retrieval', () => {
    ...
    it('should handle different METAR providers', async () => {
      ...
      expect(downloadFile).toHaveBeenCalledWith(
        expect.stringContaining('metar.vatsim.net')
      );
    });
    ...
    it('should handle empty METAR data gracefully', async () => {
      downloadFile
        .mockResolvedValueOnce(JSON.stringify(mockAirportData.valid))
        .mockResolvedValueOnce(mockMetarData.empty);
  ...
  describe('Data processing', () => {
    it('should correctly convert elevation from feet to meters', async () => {
      ...
      downloadFile
        .mockResolvedValueOnce(JSON.stringify(airportWithElevation))
        .mockResolvedValueOnce(mockMetarData.valid);
    ...
    it('should handle missing elevation data', async () => {
      ...
      downloadFile
        .mockResolvedValueOnce(JSON.stringify(airportWithoutElevation))
        .mockResolvedValueOnce(mockMetarData.valid);
    ...
    it('should filter out invalid runways', async () => {
      ...
      downloadFile
        .mockResolvedValueOnce(JSON.stringify(airportWithMixedRunways))
        .mockResolvedValueOnce(mockMetarData.valid);
  ...
  describe('URL creation functions', () => {
    it('should create correct airport URL with API token', () => {
      ...
      expect(downloadFile).toHaveBeenCalledWith(
        `https://airportdb.io/api/v1/airport/KJFK?apiToken=test-token`
      );
    });
    ...
    it('should create correct METAR URL for different providers', async () => {
      ...
      expect(downloadFile).toHaveBeenCalledWith(
        expect.stringContaining('aviationweather.gov')
      );
