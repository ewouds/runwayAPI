...
class AirportsDB {
  ...
  async fuzzySearchAirports(query, options = {}) {
    const { limit = 20, minScore = 15, includeDetails = false } = options;
  ...
  async fuzzyCitySearch(cityName, options = {}) {
    ...
    try {
      ...
      return results.sort((a, b) => {
        ...
        const aCityExact = a.airport.municipality && a.airport.municipality.toLowerCase() === cityName.toLowerCase();
        const bCityExact = b.airport.municipality && b.airport.municipality.toLowerCase() === cityName.toLowerCase();

        if (aCityExact && !bCityExact) return -1;
        if (!aCityExact && bCityExact) return 1;

        // Then prioritize city starts with query
        const aCityStarts = a.airport.municipality && a.airport.municipality.toLowerCase().startsWith(cityName.toLowerCase());
        const bCityStarts = b.airport.municipality && b.airport.municipality.toLowerCase().startsWith(cityName.toLowerCase());
