...
async function runDemo() {
    ...
    try {
        ...
        const germanLargeAirports = await db.getAirportsByCountry('DE').then(airports => 
            airports.filter(a => a.type === 'large_airport').slice(0, 5)
        );
        ...
        stats.slice(0, 10).forEach((country, index) => {
            console.log(`   ${index + 1}. ${country.country_name}: ${country.total_airports} total (${country.large_airports} large, ${country.scheduled_airports} scheduled)`);
        });
